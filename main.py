...
client_token = '7313411308:AAGVwqdpy3ITinAktvg2oCDMqAtvn1zIEhI'
...
⊛ Account ID: `{event.sender.id}`
⊛ Name: {fn}""", reply_to=event)
@client.on(events.NewMessage(pattern='/stock'))
async def handle_create(event):
    user_id = event.sender_id
    if user_id in vip:
...
⊛ **Bot** : @GatewayLookupV10Bot""")
        await client.send_message(-1002237348547,msg)
...
𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗚𝗔𝗧𝗘𝗪𝗔𝗬 𝗟𝗢𝗢𝗞-𝗨𝗣 𝗩𝟭𝟬
...
async def cmd_start(event):
    try:
        await event.respond("⚠ 𝗪𝗿𝗼𝗻𝗴 𝗰𝗼𝗺𝗺𝗮𝗻𝗱!\n𝗨𝘀𝗲 `/look discord.com` 𝘄𝗶𝘁𝗵𝗼𝘂𝘁 `https://`", reply_to=event)
    except Exception as e:
        print(e)


@client.on(events.NewMessage(pattern='/look'))
        ...
        if '.' in rw:
            ...
            if not website_url.startswith(("http://", "https://")):
                ...
                if "Error" in payment_gateways:
                    ...
                    await edit.edit(f""" ┏━━━━━━━⍟\n┃ 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻 𝗳𝗲𝘁𝗰𝗵𝗲𝗱 ✅\n┗━━━━━━━━━━━━⊛\n 𝗦𝗶𝘁𝗲 -» `{domain}`\n• 𝗚𝗮𝘁𝗲𝘄𝗮𝘆𝘀 ➜ unknown\n━━━━━━━━━━━━━━━\n[◈](https://i.ibb.co/CMcdMjf/Blue-Tosca-Geometric-Technology-Linkedln-Banner.png) 𝐓𝐢𝐦𝐞 𝐓𝐚𝐤𝐞𝐧 : `{rounded_time_taken}``s`\n[◈](https://i.ibb.co/CMcdMjf/Blue-Tosca-Geometric-Technology-Linkedln-Banner.png) 𝐁𝐨𝐭 [𝗚𝗮𝘁𝗲𝘄𝗮𝘆 𝗟𝗼𝗼𝗸𝗨𝗽 𝗩𝟭𝟬](https://t.me/GatewayLookupV10Bot)\n❝ ━━━━・{ch}・━━━━ ❞""")
                else:
                    ...
                    if event.sender.id in pre:
                        ...
                        if credit ==0:
                            await event.respond('**Credits Finished! Try /refresh** or \nBuy 𝗣𝗿𝗲𝗺𝗶𝘂𝗺 for Unlimited usage 👑.\n𝗕𝘂𝘆 ➜ [Luis](https://t.me/nb_Luis)', reply_to=event)
                        else:
                            user = event.sender
                            checked = f"[{user.first_name}](tg://user?id={user.id})" if user.username else user.first_name
                            ch_name = '『𝓒𝓱𝓪𝓷𝓷𝓮𝓵』'
